    ...
    const unsubAuth = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser) {
        ...
        onSnapshot(dmQuery, (snap) => {
          let total = 0
          snap.docs.forEach(d => {
            total += d.data().unread?.[currentUser.uid] || 0
          })
    ...
    <nav className="sticky top-0 z-50 bg-white border-b-4 border-[#FFD8A8] px-4 py-3 flex justify-between items-center shadow-sm">
      ...
      <div className="flex items-center gap-3">
        {user ? (
          ...
          <Link href="/login" className="bg-[#E67E22] text-white px-6 py-2 rounded-full font-black text-xs shadow-md active:scale-95">
            로그인
          </Link>
        )}
      </div>

  ...
  return (
    <div ref={popupRef} style={{ position: "fixed", left, top, zIndex: 9999 }}
      ...
      <div className="bg-[#FFF4E6] px-4 py-3 flex justify-between items-center border-b-2 border-[#FFD8A8]">
        ...
        <button onClick={onClose} className="w-5 h-5 flex items-center justify-center rounded-full bg-[#FFD8A8] hover:bg-[#FFB347] text-[#A64D13] text-xs font-black transition-colors">✕</button>
      </div>
      <div className="p-4">
        {loading ? (
          ...
          <div className="space-y-3">
            <div className="text-center pb-3 border-b border-[#FFE8CC]">
              ...
              {profile.server && (
                <span className="text-[11px] text-[#E67E22] font-bold bg-[#FFF4E6] px-2 py-0.5 rounded-full mt-1.5 inline-block">🗺 {profile.server} 서버</span>
              )}
            </div>
            ...
            <div className="border-t border-[#FFE8CC] pt-3 space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => handleVote("manner")} disabled={!currentUser || !!alreadyVoted || voteLoading}
                  className={`flex-1 py-2 rounded-xl text-xs font-black border-2 transition-all flex items-center justify-center gap-1 ${!currentUser || alreadyVoted ? "bg-gray-50 text-gray-300 border-gray-200 cursor-not-allowed" : "bg-green-50 text-green-700 border-green-300 hover:bg-green-100 active:scale-95 cursor-pointer"}`}>
                  👍 매너
                </button>
                <button onClick={() => handleVote("badmanner")} disabled={!currentUser || !!alreadyVoted || voteLoading}
                  className={`flex-1 py-2 rounded-xl text-xs font-black border-2 transition-all flex items-center justify-center gap-1 ${!currentUser || alreadyVoted ? "bg-gray-50 text-gray-300 border-gray-200 cursor-not-allowed" : "bg-red-50 text-red-600 border-red-300 hover:bg-red-100 active:scale-95 cursor-pointer"}`}>
                  👎 비매너
                </button>
              </div>
...
function AdminModal({ targetMsg, adminUid, room, onClose }: {
  ...
  const handleMute = async () => {
    ...
    setLoading(true)
    try {
      await muteUser(targetMsg.uid, adminUid, muteHours, reason)
      setDone(`✅ ${targetMsg.displayName}님을 ${muteHours}시간 추방했어요`)
  ...
  const handleBan = async () => {
    ...
    setLoading(true)
    try {
      await banUser(targetMsg.uid, adminUid, reason)
      setDone(`✅ ${targetMsg.displayName}님을 영구 차단했어요`)
      setTimeout(onClose, 1500)
    } catch { setDone("❌ 차단 실패") }
    finally { setLoading(false) }
  }
  ...
  const handleWarn = async () => {
    ...
    setLoading(true)
    try {
      await sendWarning(targetMsg.uid, targetMsg.displayName, adminUid, reason, room)
      setDone("✅ 경고 메시지를 전송했어요")
      setTimeout(onClose, 1200)
    } catch { setDone("❌ 경고 전송 실패") }
    finally { setLoading(false) }
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999]"
      onClick={onClose}>
      <div ref={modalRef} className="w-80 bg-white rounded-2xl shadow-2xl overflow-hidden border-2 border-red-200"
        ...
        <div className="bg-red-50 px-4 py-3 border-b-2 border-red-200 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-red-400 font-bold">— {targetMsg.displayName}</span>
          </div>
          <button onClick={onClose} className="w-5 h-5 flex items-center justify-center rounded-full bg-red-100 hover:bg-red-200 text-red-500 text-xs font-black">✕</button>
        </div>

        <div className="p-4">
          {done ? (
            <p className="text-center font-black text-sm py-4">{done}</p>
          ) : loading ? (
            <div className="flex justify-center py-6"><div className="w-6 h-6 border-2 border-red-500 border-t-transparent rounded-full animate-spin" /></div>
          ) : step === "menu" ? (
            /* 메뉴 선택 */
            <div className="space-y-2">
              <button onClick={handleDeleteMessage}
                className="w-full text-left px-4 py-3 rounded-xl bg-gray-50 hover:bg-gray-100 border-2 border-gray-200 text-sm font-bold text-gray-700 transition-colors flex items-center gap-2">
                🗑️ 이 메시지 삭제
              </button>
              <button onClick={() => setStep("warn")}
                className="w-full text-left px-4 py-3 rounded-xl bg-yellow-50 hover:bg-yellow-100 border-2 border-yellow-200 text-sm font-bold text-yellow-700 transition-colors flex items-center gap-2">
                ⚠️ 경고 메시지 전송
              </button>
              <button onClick={() => setStep("mute")}
                className="w-full text-left px-4 py-3 rounded-xl bg-orange-50 hover:bg-orange-100 border-2 border-orange-200 text-sm font-bold text-orange-700 transition-colors flex items-center gap-2">
                🔇 일시 추방 (채팅 금지)
              </button>
              <button onClick={() => setStep("ban")}
                className="w-full text-left px-4 py-3 rounded-xl bg-red-50 hover:bg-red-100 border-2 border-red-200 text-sm font-bold text-red-700 transition-colors flex items-center gap-2">
                🚫 영구 차단
              </button>
            </div>
          ) : step === "warn" ? (
            /* 경고 */
            <div className="space-y-3">
              <button onClick={() => setStep("menu")} className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1">← 뒤로</button>
              <p className="font-black text-sm text-yellow-700">⚠️ 경고 메시지 전송</p>
              <textarea
                className="w-full p-3 rounded-xl border-2 border-yellow-200 text-sm font-bold outline-none focus:border-yellow-400 resize-none bg-yellow-50"
                placeholder="경고 내용을 입력하세요" rows={3}
                value={reason} onChange={(e) => setReason(e.target.value)} />
              <button onClick={handleWarn}
                className="w-full py-2.5 bg-yellow-500 hover:bg-yellow-600 text-white rounded-xl font-black text-sm transition-colors">
                전송하기
              </button>
            </div>
          ) : step === "mute" ? (
            ...
            <div className="space-y-3">
              ...
              <div>
                <p className="text-xs font-bold text-gray-500 mb-1">추방 시간</p>
                <div className="flex gap-2">
                  {[1, 3, 6, 24].map(h => (
                    <button key={h} onClick={() => setMuteHours(h)}
                      className={`flex-1 py-2 rounded-xl text-xs font-black border-2 transition-colors ${muteHours === h ? "bg-orange-500 text-white border-orange-500" : "bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-100"}`}>
                      {h}시간
                    </button>
                  ))}
                </div>
              ...
              <textarea
                className="w-full p-3 rounded-xl border-2 border-orange-200 text-sm font-bold outline-none focus:border-orange-400 resize-none bg-orange-50"
                placeholder="추방 사유를 입력하세요" rows={2}
                value={reason} onChange={(e) => setReason(e.target.value)} />
              <button onClick={handleMute}
                className="w-full py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-black text-sm transition-colors">
                {muteHours}시간 추방하기
              </button>
            </div>
          ) : (
            ...
            <div className="space-y-3">
              ...
              <div className="p-3 bg-red-50 rounded-xl border-2 border-red-200">
                <p className="text-xs font-bold text-red-600">⚠️ 이 작업은 되돌리기 어려워요. 신중하게 결정해주세요.</p>
              </div>
              <textarea
                className="w-full p-3 rounded-xl border-2 border-red-200 text-sm font-bold outline-none focus:border-red-400 resize-none bg-red-50"
                placeholder="차단 사유를 입력하세요" rows={2}
                value={reason} onChange={(e) => setReason(e.target.value)} />
              <button onClick={handleBan}
                className="w-full py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl font-black text-sm transition-colors">
                영구 차단하기
              </button>
            </div>
          )}
...
function ContextMenu({ pos, targetMsg, currentUid, isAdminUser, onClose, onViewProfile, onStartDM, onAdminAction }: {
  ...
  const top = Math.min(pos.y, window.innerHeight - 150)
...
export default function ChatRoom({ room = "mapleland_trade" }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        if (banned) {
          setBlocked({ banned: true, muted: false })
          return
        }
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if (blocked?.muted) {
      const until = blocked.until?.toLocaleString("ko-KR") || ""
      alert(`${until}까지 채팅이 금지된 상태예요.`)
      return
    }
  ...
  const handleViewProfile = useCallback((msg: Message, pos: { x: number; y: number }) => {
    setContextMenu(null)
    requestAnimationFrame(() => setProfilePopup({ uid: msg.uid, displayName: msg.displayName, isGuest: msg.isGuest, pos }))
  }, [])
    ...
    <div className="flex flex-col h-[650px] bg-white border-4 border-[#FFD8A8] rounded-[35px] overflow-hidden shadow-xl"
      ...
      {blocked?.muted && (
        <div className="bg-orange-100 border-b-2 border-orange-300 px-4 py-2 text-xs font-black text-orange-700 text-center">
          🔇 {blocked.until?.toLocaleString("ko-KR")}까지 채팅이 금지됐어요
        </div>
      )}
            ...
            {msg.isSystem ? (
              ...
              <>
                <div className="flex items-center gap-1.5 mb-1 text-[10px] font-bold text-[#A64D13]">
                  <span className={`select-none ${msg.uid !== user?.uid ? "cursor-pointer hover:underline" : ""}`}
                  ...
                  {isAdminUser && (
                    <button onClick={() => deleteMessage(msg.id)}
                      className="ml-1 text-red-300 hover:text-red-500 transition-colors text-[10px]" title="메시지 삭제">
                      ✕
                    </button>
                  )}
                </div>
      ...
      {/* 입력창 */}
      <form onSubmit={sendMessage} className="p-4 bg-[#FFF4E6] border-t-4 border-[#FFD8A8] flex flex-col gap-2"
        onClick={(e) => e.stopPropagation()}>
        {!user && (
          <input className="w-full p-2.5 rounded-xl border-2 border-[#FFD8A8] font-bold text-sm outline-none focus:border-[#E67E22] bg-white"
            placeholder="닉네임을 입력하세요 (비회원)" value={guestName}
        ...
        <div className="flex gap-2 items-center">
          ...
          <input className="flex-1 p-3 rounded-2xl border-2 border-[#FFD8A8] font-bold text-sm outline-none focus:border-[#E67E22]"
            placeholder={blocked?.banned ? "채팅이 차단됐어요" : blocked?.muted ? "채팅이 금지됐어요" : "거래 내용을 입력하세요!"}
            value={newMessage} onChange={(e) => setNewMessage(e.target.value)}
            disabled={!!blocked?.banned || !!blocked?.muted} />
          <button disabled={!!blocked?.banned || !!blocked?.muted}
            className="bg-[#E67E22] disabled:bg-gray-300 text-white px-6 py-3 rounded-2xl font-black text-sm shadow-md active:scale-95 whitespace-nowrap transition-colors">
            전송
          </button>
        </div>
      </form>

...
export default function BoardList({ initialPosts }: { initialPosts: Post[] }) {
  const { lang } = useLang()
  const bt = boardT[lang]
  const [posts, setPosts] = useState<Post[]>(initialPosts)  // ← 서버에서 받은 초기값
  ...
  // onSnapshot 실시간 구독 (초기값은 initialPosts, 마운트 후 실시간으로 갱신)
  useEffect(() => {

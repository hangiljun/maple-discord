...
// 이미지에서 배경 제거: Phase1 = 가장자리 BFS(다크픽셀), Phase2 = 고립된 회색 패치 제거
function BGCanvas({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // 다크 배경 픽셀 판별 (#07090f 계열)
      const isDark = (i: number) => {
        const avg = (d[i] + d[i+1] + d[i+2]) / 3
        const chroma = Math.max(d[i], d[i+1], d[i+2]) - Math.min(d[i], d[i+1], d[i+2])
        return avg < 40 && chroma < 18
      }

      // Phase 1: 가장자리 4면에서 BFS — 연결된 다크 배경 제거
      const visited = new Uint8Array(w * h)
      const queue: number[] = []
      const enq = (idx: number) => {
        if (idx < 0 || idx >= w * h || visited[idx] || !isDark(idx * 4)) return
        visited[idx] = 1
        queue.push(idx)
      }
      ...
      // Phase 2: 고립된 회색/체커보드 패치 제거 (캐릭터 내부 잔여 픽셀)
      const isGray = (i: number) => {
        if (d[i + 3] === 0) return false
        const avg = (d[i] + d[i+1] + d[i+2]) / 3
        const chroma = Math.max(d[i], d[i+1], d[i+2]) - Math.min(d[i], d[i+1], d[i+2])
        return avg > 110 && chroma < 28
      }

      const unvisited = new Uint8Array(w * h)
      for (let i = 0; i < w * h; i++) {
        if (isGray(i * 4)) unvisited[i] = 1
      }

      for (let start = 0; start < w * h; start++) {
        if (!unvisited[start]) continue
        ...
        while (bqi < blob.length) {
          ...
          const x = idx % w, y = Math.floor(idx / w)
          const chk = (n: number) => {
            if (n >= 0 && n < w * h && unvisited[n]) { unvisited[n] = 0; blob.push(n) }
          }
          if (x > 0) chk(idx - 1)
          if (x < w - 1) chk(idx + 1)
          if (y > 0) chk(idx - w)
        ...
        // 3000px 미만 고립 패치는 배경으로 간주 제거
        if (blob.length < 3000) {
          for (const bi of blob) d[bi * 4 + 3] = 0
        }
      }
      ...
      <section className="relative overflow-hidden min-h-[92vh] flex items-center" style={{ background: "#07090f" }}>
        ...
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute top-[-10%] left-[-5%] w-[700px] h-[600px] rounded-full"
            style={{ background: "radial-gradient(ellipse, rgba(99,70,210,0.3) 0%, transparent 70%)" }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="inline-flex items-center gap-2 rounded-full px-5 py-2 mb-7 text-sm font-medium text-[#a5b4fc]"
                style={{ background: "rgba(99,70,210,0.18)", border: "1px solid rgba(139,92,246,0.3)" }}>
              ...
              <div className="flex gap-2 flex-wrap justify-center mb-8">
                {[
                  { label: "🍁 메이플스토리", color: "#f59e0b", bg: "rgba(245,158,11,0.15)", border: "rgba(245,158,11,0.3)" },
                  { label: "🌿 메이플랜드",   color: "#4ade80", bg: "rgba(74,222,128,0.12)",  border: "rgba(74,222,128,0.3)" },
                  { label: "🪐 메이플플래닛", color: "#c084fc", bg: "rgba(192,132,252,0.15)", border: "rgba(192,132,252,0.3)" },
                ].map(t => (
                  <span key={t.label} className="text-xs font-bold px-3.5 py-1.5 rounded-full"
              ...
              <div className="grid grid-cols-3 gap-3 mb-10 w-full max-w-md">
                <div className="flex flex-col items-center gap-1 px-3 py-4 rounded-2xl overflow-hidden"
                  ...
                  <p className="text-2xl font-black text-white leading-none">50,000+</p>
                ...
                <div className="flex flex-col items-center gap-2 px-3 py-5 rounded-2xl"
                  ...
                  <p className="text-2xl font-black text-white leading-none">3개</p>
                ...
                <div className="flex flex-col items-center gap-2 px-3 py-5 rounded-2xl"
                  ...
                  <div className="w-14 h-14 rounded-full flex items-center justify-center mb-1"
                    style={{ background: "rgba(34,211,238,0.1)", border: "2px solid rgba(34,211,238,0.6)",
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <a href={DISCORD_URL} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ background: "#5865F2", boxShadow: "0 0 24px rgba(88,101,242,0.5)" }}>
                  <DiscordIcon />디스코드 참여하기
                </a>
                <Link href="/notice"
                  className="flex items-center gap-2 font-semibold px-7 py-3.5 rounded-xl text-base text-white transition-all hover:bg-white/10"
            ...
            {/* 오른쪽: 캐릭터 — Canvas로 배경 제거 후 렌더링 */}
            <div className="hidden lg:flex items-center justify-center relative"
              style={{ height: "85vh", maxHeight: "800px" }}>
              {/* sky-blue 글로우 */}
              <div className="absolute inset-0 pointer-events-none"
                style={{ background: "radial-gradient(ellipse 60% 70% at 60% 50%, rgba(125,211,252,0.15) 0%, transparent 65%)" }} />
              <BGCanvas className="h-full w-auto relative z-10" />
              {/* 왼쪽 페이드 */}
              <div className="absolute inset-0 pointer-events-none"
                style={{ background: "linear-gradient(to right, #07090f 0%, #07090f 5%, transparent 28%)" }} />
            </div>
      ...
      <section className="relative overflow-hidden border-t border-white/[0.06]">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[700px] h-[400px]"
            style={{ background: "radial-gradient(ellipse, rgba(88,101,242,0.18) 0%, transparent 70%)" }} />
        </div>
        <div className="relative max-w-4xl mx-auto px-4 py-24 text-center">
          ...
          <a href={DISCORD_URL} target="_blank" rel="noopener noreferrer"
            ...
            style={{ background: "#5865F2", boxShadow: "0 0 32px rgba(88,101,242,0.5)" }}>
            <DiscordIcon />디스코드 참여하기
          </a>
        </div>
      </section>

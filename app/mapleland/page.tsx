import type { Metadata } from 'next'

export const metadata: Metadata = {
  title: '메이플랜드 디스코드 거래방·파티모집',
  description: '메이플랜드 디스코드 거래방. 메랜 메소·아이템 거래와 파티 모집, 클래식 통합 커뮤니티까지. 신고·인증 시스템으로 안전한 메이플랜드 거래.',
  alternates: { canonical: 'https://www.maplediscord.com/mapleland' },
  openGraph: {
    url: '/mapleland',
    title: '메이플랜드 디스코드 거래방·파티모집',
    description: '메이플랜드 디스코드 거래방. 메랜 메소·아이템 거래와 파티 모집, 클래식 통합 커뮤니티까지.',
  },
}

const breadcrumbJsonLd = {
  "@context": "https://schema.org",
  "@type": "BreadcrumbList",
  "itemListElement": [
    { "@type": "ListItem", "position": 1, "name": "홈", "item": "https://www.maplediscord.com" },
    { "@type": "ListItem", "position": 2, "name": "메이플랜드 디스코드" }
  ]
}

export default function MapleLandPage() {
  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbJsonLd) }}
      />

      {/* 히어로 */}
      <section className="relative bg-gradient-to-b from-blue-50 to-white">
        <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-100 border border-blue-200 text-blue-700 text-sm font-semibold mb-6">
              🌿 MapleLand
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-gray-900 mb-4">
              메이플랜드 디스코드
            </h1>
            <h2 className="text-lg md:text-xl text-gray-600 font-medium">
              메랜 거래방 · 파티 모집 · 클래식 커뮤니티
            </h2>
          </div>

          {/* 히어로 이미지 placeholder */}
          <div className="relative w-full max-w-4xl mx-auto rounded-2xl overflow-hidden border border-gray-200 shadow-lg bg-gradient-to-br from-blue-100 to-cyan-100">
            <div className="w-full aspect-video flex items-center justify-center">
              <div className="text-center">
                <div className="text-6xl mb-4">🌿</div>
                <p className="text-gray-600 font-medium">메이플랜드 디스코드 거래방·파티모집</p>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* SEO 텍스트 */}
      <section className="max-w-4xl mx-auto px-4 py-16">
        <div className="space-y-4">
          <p className="text-gray-700 text-base leading-relaxed">
            메이플랜드 디스코드는 메랜 유저를 위한 <strong className="text-gray-900">거래방과 파티 모집</strong> 채널을 중심으로 운영됩니다. 메랜 거래방에서 클래식 서버의 메소·아이템을 유저끼리 직접 거래하고, 파티 모집 채널에서 사냥·보스 파티원을 바로 구할 수 있습니다.
          </p>
          <p className="text-gray-700 text-base leading-relaxed">
            공지사항, 자유대화, 가이드, 닉변요청까지 <strong className="text-gray-900">클래식 통합 채널</strong>에서 메이플랜드 정보를 한눈에 확인할 수 있습니다. 신고문의와 인증 시스템으로 메이플랜드 거래 사기를 예방합니다.
          </p>
        </div>
      </section>

      {/* 채널 기능 카드 */}
      <section className="bg-gray-50 py-16">
        <div className="max-w-6xl mx-auto px-4">
          <h3 className="text-2xl md:text-3xl font-black text-center mb-12 text-gray-900">
            메이플랜드 디스코드 채널
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* 카드 1 */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition-shadow">
              <div className="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center mb-4">
                <span className="text-2xl">💰</span>
              </div>
              <h4 className="text-lg font-bold text-gray-900 mb-2">메랜 거래방</h4>
              <p className="text-sm text-gray-600 leading-relaxed">
                메이플랜드 메소·아이템 거래 전용
              </p>
            </div>

            {/* 카드 2 */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition-shadow">
              <div className="w-12 h-12 rounded-xl bg-green-100 flex items-center justify-center mb-4">
                <span className="text-2xl">👥</span>
              </div>
              <h4 className="text-lg font-bold text-gray-900 mb-2">파티 모집</h4>
              <p className="text-sm text-gray-600 leading-relaxed">
                사냥·보스 파티, 파티원 구인
              </p>
            </div>

            {/* 카드 3 */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition-shadow">
              <div className="w-12 h-12 rounded-xl bg-cyan-100 flex items-center justify-center mb-4">
                <span className="text-2xl">💬</span>
              </div>
              <h4 className="text-lg font-bold text-gray-900 mb-2">통합 커뮤니티</h4>
              <p className="text-sm text-gray-600 leading-relaxed">
                공지사항, 자유대화, 가이드, 닉변요청
              </p>
            </div>

            {/* 카드 4 */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition-shadow">
              <div className="w-12 h-12 rounded-xl bg-red-100 flex items-center justify-center mb-4">
                <span className="text-2xl">🔒</span>
              </div>
              <h4 className="text-lg font-bold text-gray-900 mb-2">안전 거래</h4>
              <p className="text-sm text-gray-600 leading-relaxed">
                신고문의·인증으로 사기 예방
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="max-w-4xl mx-auto px-4 py-16 text-center">
        <a
          href="https://discord.gg/2UwBw8dnSv"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-3 px-8 py-4 bg-blue-500 hover:bg-blue-600 text-white font-bold text-lg rounded-xl transition-all hover:-translate-y-1 shadow-lg"
        >
          <svg width="24" height="24" viewBox="0 0 71 55" fill="currentColor">
            <path d="M60.1 4.9A58.5 58.5 0 0 0 45.5.4a.2.2 0 0 0-.2.1 40.7 40.7 0 0 0-1.8 3.7 54 54 0 0 0-16.2 0A37.8 37.8 0 0 0 25.5.5a.2.2 0 0 0-.2-.1A58.3 58.3 0 0 0 10.7 4.9a.2.2 0 0 0-.1.1C1.6 18.1-.9 31 .3 43.6a.2.2 0 0 0 .1.2 58.8 58.8 0 0 0 17.7 8.9.2.2 0 0 0 .2-.1 42 42 0 0 0 3.6-5.9.2.2 0 0 0-.1-.3 38.7 38.7 0 0 1-5.5-2.6.2.2 0 0 1 0-.4l1.1-.8a.2.2 0 0 1 .2 0c11.5 5.3 24 5.3 35.4 0a.2.2 0 0 1 .2 0l1.1.8a.2.2 0 0 1 0 .4 36.1 36.1 0 0 1-5.6 2.6.2.2 0 0 0-.1.3 47 47 0 0 0 3.6 5.9.2.2 0 0 0 .2.1 58.6 58.6 0 0 0 17.8-8.9.2.2 0 0 0 .1-.2c1.4-14.7-2.4-27.5-10.1-38.8a.2.2 0 0 0-.1 0ZM23.7 36.3c-3.5 0-6.4-3.2-6.4-7.1s2.8-7.1 6.4-7.1c3.6 0 6.5 3.2 6.4 7.1 0 3.9-2.8 7.1-6.4 7.1Zm23.7 0c-3.5 0-6.4-3.2-6.4-7.1s2.8-7.1 6.4-7.1c3.6 0 6.5 3.2 6.4 7.1 0 3.9-2.8 7.1-6.4 7.1Z"/>
          </svg>
          메이플랜드 디스코드 입장하기 →
        </a>
      </section>
    </>
  )
}
